    ...
    final databaseOptions = sqflite.OpenDatabaseOptions(
      ...
      onCreate: (database, version) async {
        await database.execute(
            'CREATE TABLE IF NOT EXISTS `products` (`id` TEXT, `name` TEXT NOT NULL, `description` TEXT NOT NULL, `image` TEXT NOT NULL, `inStock` INTEGER NOT NULL, `price` REAL NOT NULL, `quantity` INTEGER, `discount` REAL NOT NULL, PRIMARY KEY (`id`))');
...
class _$CartDao extends CartDao {
  ...
  @override
  Future<List<Product>> getProductCartStatus(String id) async {
    return _queryAdapter.queryList('SELECT * FROM products WHERE id=?1',
        mapper: (Map<String, Object?> row) => Product(
            id: row['id'] as String?,
            quantity: row['quantity'] as int?,
            name: row['name'] as String,
  ...
  Future<void> updateCartProduct(Product product) async {
    await _productUpdateAdapter.update(product, OnConflictStrategy.replace);
  }
}
